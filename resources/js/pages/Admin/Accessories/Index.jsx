...
export default function Index({ accessories, filters }) {
    ...
    function confirmDelete() {
        ...
        setDeleting(true);
        router.delete(`/admin/accessories/${deleteTarget.id}`, {
            onFinish: () => {
                setDeleting(false);
    ...
    function clearFilters() {
        setSearch('');
        setStatus('');
        router.get('/admin/accessories', {}, { preserveState: true, replace: true });
        setFilterOpen(false);
    }
    ...
    return (
        <AdminLayout>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Accessories</h1>
                    <p className="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">
                        Spare parts aur accessories manage karo.
                    </p>
                </div>
                    ...
                    {hasActiveFilters ? (
                        <>
                            <p className="text-sm text-[#706f6c] dark:text-[#A1A09A]">
                                Filter se koi accessory nahi mili.
                            </p>
                            <button
                        ...
                        <>
                            <p className="text-sm text-[#706f6c] dark:text-[#A1A09A]">Abhi koi accessory nahi hai.</p>
                            <Link
                                href="/admin/accessories/create"
                                className="mt-3 inline-block cursor-pointer text-sm font-medium text-emerald-600 hover:underline"
                            >
                                Pehli accessory add karo →
                            </Link>
                        </>
                    )}
                </div>
                    ...
                    <div className="hidden overflow-hidden rounded-xl border border-[#e3e3e0] bg-white md:block dark:border-[#3E3E3A] dark:bg-[#161615]">
                        <table className="w-full text-left text-sm">
                            <thead className="border-b border-[#e3e3e0] bg-[#FAFAF9] text-xs text-[#706f6c] uppercase dark:border-[#3E3E3A] dark:bg-[#1c1c1a] dark:text-[#A1A09A]">
                                <tr>
                                    <th className="px-5 py-3 font-medium">Accessory</th>
                            ...
                            <tbody className="divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                                {accessories.data.map((accessory) => (
                                    <tr key={accessory.id}>
                                        ...
                                        <td className="max-w-xs truncate px-5 py-3 text-[#706f6c] dark:text-[#A1A09A]">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <p className="font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{accessory.name}</p>
                                        <p className="line-clamp-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">
                                            {accessory.description || '—'}
                                        </p>
                                    </div>
                                    <span
