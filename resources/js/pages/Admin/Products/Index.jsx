...
export default function Index({ products, categories, filters }) {
    ...
    function confirmDelete() {
        ...
        setDeleting(true);
        router.delete(`/admin/products/${deleteTarget.slug}`, {
            onFinish: () => {
                setDeleting(false);
    ...
    function applyFilters() {
        router.get(
            '/admin/products',
            { search: search || undefined, category_id: categoryId || undefined, status: status || undefined },
    ...
    return (
        <AdminLayout>
            ...
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[#1b1b18] dark:text-[#EDEDEC]">Products</h1>
                    <p className="mt-1 text-sm text-[#706f6c] dark:text-[#A1A09A]">Scooters manage karo.</p>
                </div>
                    ...
                    {hasActiveFilters ? (
                        <>
                            <p className="text-sm text-[#706f6c] dark:text-[#A1A09A]">
                                Filter se koi product nahi mila.
                            </p>
                            <button
                        ...
                        <>
                            <p className="text-sm text-[#706f6c] dark:text-[#A1A09A]">Abhi koi product nahi hai.</p>
                            <Link
                                href="/admin/products/create"
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-[#e3e3e0] dark:divide-[#3E3E3A]">
                                {products.data.map((product) => (
                                    <tr
                                        key={product.id}
                                        onClick={() => router.visit(`/admin/products/${product.slug}`)}
                                        className="cursor-pointer hover:bg-[#FAFAF9] dark:hover:bg-[#1c1c1a]"
                                    >
                                        <td className="flex items-center gap-3 px-5 py-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-[#1b1b18] dark:text-[#EDEDEC]">{product.name}</p>
                                                {product.is_featured && (
                                                    <span className="text-xs text-emerald-600">Featured</span>
                                                )}
                                            </div>
                                        </td>
                    ...
                    <div className="grid gap-3 md:hidden">
                        {products.data.map((product) => (
                            <div
                                key={product.id}
                                onClick={() => router.visit(`/admin/products/${product.slug}`)}
                                className="cursor-pointer rounded-xl border border-[#e3e3e0] bg-white p-4 dark:border-[#3E3E3A] dark:bg-[#161615]"
                            >
                                <div className="flex items-center gap-3">
                                    {product.primary_image_url ? (
                                        <img
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-[#706f6c] dark:text-[#A1A09A]">
                                            {product.category?.name} · ₹{Number(product.price).toLocaleString('en-IN')}
                                        </p>
                                    </div>
                                    <span
                                        className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                                ...
                                <div
                                    className="mt-3 flex gap-3 border-t border-[#e3e3e0] pt-3 dark:border-[#3E3E3A]"
            ...
            <ConfirmDialog
                ...
                description={`"${deleteTarget?.name}" ko delete karne se ye permanently hat jayega. Ye action wapas nahi ho sakta.`}
